...
class JanusHubSupabaseUser extends BaseAuthUser {
  JanusHubSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> janusHubSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = JanusHubSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
